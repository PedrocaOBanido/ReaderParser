...
2. **Identify** which subagent owns it (`source-author`, `screen-author`,
   `room-migration`, `reviewer`) — or say "answering directly" for a
   read-only question, or "no single owner" if it spans multiple subagents.
...
4. **Plan**: list files in scope, the AGENTS.md constraints that apply,
   ...
   - Skip the wait when: the request is read-only (answer directly), or
     ...
     already present.
5. After approval, **dispatch** via the `task` tool using the handoff
   template in your system prompt.
6. After the subagent reports done, **auto-dispatch `@reviewer`** against
   the uncommitted diff and surface findings grouped by severity. Do not
   auto-fix — that's a fresh turn.
