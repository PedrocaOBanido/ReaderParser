---
name: domain-author
description: Authors domain models, the Source interface, HtmlSource base class, SourceRegistry, and computeSourceId. Follows TDD — tests first, then implementations. Routing keywords — domain, model, interface, contract, Source, HtmlSource, registry, computeSourceId, TDD, data class, sealed, immutable.
...
- `HtmlSourceTest.kt` — tests the default implementations in HtmlSource
  using a minimal concrete subclass with MockEngine-backed HttpClient.
...
1. `architecture.md` — full Source interface, HtmlSource, domain models, computeSourceId.
2. `data/source/AGENTS.md` — Source contract rules.
3. `testutil/KtorMockHelpers.kt` and `testutil/MainDispatcherRule.kt`.
