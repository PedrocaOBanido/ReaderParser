...
Dispatch the `room-migration` agent via the `Agent` tool with
`subagent_type: "room-migration"` and the following brief:
...
>    with explicit SQL for the change. Do not use any auto-migration
>    helpers.
...
> 7. Run `./gradlew :app:assembleDebug` to regenerate the schema JSON;
>    verify the new file appears in `app/schemas/`.
>
> Never use `fallbackToDestructiveMigration` to skip writing the
> migration.
